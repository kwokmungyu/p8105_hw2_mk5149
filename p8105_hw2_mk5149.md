...
We’ll read and clean the data for each of the three trash wheels from
the Excel file.

The process involves reading the specific sheet, cleaning the column
names to a standard format, removing rows that aren’t dumpster
collections, and adding a new column to identify which trash wheel
collected the trash.
